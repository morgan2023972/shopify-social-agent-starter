...
function readClient() {
  if (!env.X_BEARER_TOKEN) {
    throw new Error('X_BEARER_TOKEN missing. Required for reading public posts.');
  }
  return new TwitterApi(env.X_BEARER_TOKEN).readOnly;
}

function writeClient() {
  const required = [env.X_APP_KEY, env.X_APP_SECRET, env.X_ACCESS_TOKEN, env.X_ACCESS_SECRET];
  if (required.some((v) => !v)) {
    throw new Error('X write credentials missing. Need X_APP_KEY, X_APP_SECRET, X_ACCESS_TOKEN, X_ACCESS_SECRET.');
  }
...
export async function fetchRecentUserPosts(handle: string, maxResults = 5): Promise<SocialPost[]> {
  const client = readClient();
...
export async function publishXReply(text: string, replyToPostId: string): Promise<{ id: string }> {
